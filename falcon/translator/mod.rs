...
pub trait Arch {
    ...
    fn translate_function(&self, bytes: &[u8], function_address: u64) -> Result<Function> {
        ...
        while translation_queue.len() > 0 {
            ...
            // get the bytes for this translation
            let start = (block_address - function_address) as usize;
            let block_bytes = match bytes.get(start..bytes.len()) {
                Some(block_bytes) => block_bytes,
                None => bail!("out of bounds in function translation")
            };
